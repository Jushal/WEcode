...
function FilesView() {
    ...
    return (
        ...
        >
            {/* Header */}
            <div className="flex items-center gap-2 pb-2 border-b border-gray-700 mb-1">
                <div className="w-2 h-2 rounded-full bg-primary animate-pulse" />
                <h1 className="text-sm font-bold text-white tracking-wider uppercase">Files</h1>
            </div>

            <FileStructureView />

            <div className={cn(`flex min-h-fit flex-col justify-end pt-2 gap-1`, {
                hidden: minHeightReached,
            })}>
                <hr className="border-gray-700" />
                <button
                    className="mt-1 flex w-full items-center gap-2 rounded-xl p-2 text-sm text-gray-300 transition-all hover:bg-darkHover hover:text-white"
                    onClick={handleOpenDirectory}
                    disabled={isLoading}
                >
                    <TbFileUpload size={18} className="text-primary" />
                    {isLoading ? "Loading..." : "Open File/Folder"}
                </button>
                <button
                    className="flex w-full items-center gap-2 rounded-xl p-2 text-sm text-gray-300 transition-all hover:bg-darkHover hover:text-white"
                    onClick={downloadFilesAndFolders}
                >
                    <BiArchiveIn size={18} className="text-primary" />
                    Download Code
                </button>
            </div>
        </div>
