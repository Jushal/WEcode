...
app.post("/api/run", express.json(), async (req: Request, res: Response) => {
    try {
   ...
   const compilerMap: Record<string, string> = {
    ...
    rust: "rust-1.93",
        ...
        const compiler = compilerMap[language] || "nodejs-22"
...
const response = await fetch("https://api.onlinecompiler.io/api/run-code-sync/", {
            ...
            headers: {
                ...
                "Authorization": "5d742f355294a6bc1d8f1ca78d6a814b"
            },
            body: JSON.stringify({
                compiler: compiler,
                code: code,
        ...
        res.json({ stdout: result.output || "", stderr: result.error || "" })
    } catch (error) {
